#!/bin/sh
# Comprehensive tests for proxychains4-cli
# Run from project root: ./tests/cli/test-cli.sh

set -e
cd "$(dirname "$0")/../.."

CLI="./src/proxychains4-cli"
PASS=0
FAIL=0
...
echo "=== proxychains4-cli Test Suite ==="
echo ""
...
output=$($CLI -v 2>&1)
check_contains "-v shows version" "proxychains4-cli" "$output"

output=$($CLI --version 2>&1)
check_contains "--version shows version" "proxychains4-cli" "$output"
...
# ============================================================
echo ""
echo "--- Proxy Options (URL Syntax) ---"
# ============================================================

# Basic URL syntax
output=$($CLI -P socks5://127.0.0.1:1080 --config 2>&1)
check_contains "-P socks5 URL" "socks5" "$output"
check_contains "-P IP" "127.0.0.1" "$output"
check_contains "-P port" "1080" "$output"

output=$($CLI -P socks4://10.0.0.1:9050 --config 2>&1)
check_contains "-P socks4 URL" "socks4" "$output"

output=$($CLI -P http://192.168.1.1:8080 --config 2>&1)
check_contains "-P http URL" "http" "$output"

output=$($CLI -P raw://10.0.0.1:12345 --config 2>&1)
check_contains "-P raw URL" "raw" "$output"

# Proxy with user/pass in URL
output=$($CLI -P socks5://user:pass@127.0.0.1:1080 --config 2>&1)
check_contains "-P URL with auth user" "user" "$output"
check_contains "-P URL with auth pass" "pass" "$output"

# Special chars in password (URL encoded: %40=@ %3A=: %25=%)
output=$($CLI -P 'socks5://user:p%40ss%3Aword@127.0.0.1:1080' --config 2>&1)
check_contains "-P URL encoded @ in pass" "p@ss:word" "$output"
...
# Invalid protocol
output=$($CLI -P invalid://127.0.0.1:1080 --config 2>&1) || true
check_contains "invalid protocol error" "Invalid proxy protocol" "$output"

# Missing port
output=$($CLI -P socks5://127.0.0.1 --config 2>&1) || true
check_contains "missing port error" "Invalid or missing port" "$output"
...
check_not_contains "curl -v not treated as --version" "proxychains4-cli" "$output"
...
# Only run if proxychains4 binary exists (dev mode)
